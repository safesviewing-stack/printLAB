...
const SUPABASE_ANON_KEY = "sb_publishable_kelRYJ3Fa56DXY3GhxWLHQ_YiLJzStR";
...
authHeaderStyles.textContent = `
  ...
  @media(max-width:768px){
    .user-header-info{ gap:8px; }
    .user-greeting{ font-size:13px; }
    .credits-badge{ padding:4px 10px; min-height:auto; font-size:13px; border-radius:9999px; }
    .user-header-info .btn-auth.light{ padding:8px 14px !important; font-size:13px !important; }
  }
`;
...
async function initAuth() {
        ...
        if (authButtons) {
          authButtons.innerHTML = `
            <div class="user-header-info">
              <span class="user-greeting">Hola, ${cachedName}</span>
              <span class="credits-badge">Créditos: ${cachedCredits}</span>
              <button id="btn-logout" class="btn-auth light" type="button" onclick="handleLogout()">Cerrar Sesión</button>
            </div>
          `;
  ...
  if (user) {
    let profile = null;
    try {
      const result = await supabaseClient.from("profiles").select("full_name, credits, plan").eq("id", user.id).single();
    ...
    if (window.location.pathname.toLowerCase().includes("checkout-success.html")) {
      ...
      if (!isNaN(cached) && cached > credits) credits = cached;
    }
    ...
    if (authButtons) {
      authButtons.innerHTML = `
        <div class="user-header-info">
          ...
          <span class="credits-badge">Créditos: ${credits}</span>
          <button id="btn-logout" class="btn-auth light" type="button" onclick="handleLogout()">Cerrar Sesión</button>
        </div>
      `;
      authButtons.style.opacity = "1";
  ...
  if (authButtons) {
    authButtons.innerHTML = `
      <a href="${pathPrefix}login.html" class="btn-auth light" onclick="localStorage.removeItem('sb-jmwprzgfdkphbxryjbnr-auth-token');">Iniciar Sesión</a>
      <a href="${pathPrefix}registro.html" id="header-register-link" class="btn-auth green" onclick="localStorage.removeItem('sb-jmwprzgfdkphbxryjbnr-auth-token');">Registrarse</a>
    `;

    const headerRegLink = document.getElementById("header-register-link");
    if (headerRegLink) {
      headerRegLink.href = `${pathPrefix}registro.html`;
    }
  ...
  // ===================================================
  // REDIRECCIÓN OBLIGATORIA PARA PÁGINAS PRIVADAS
  // ===================================================
  if (currentPathname.includes("informe-stl")) {
    window.location.href = pathPrefix + "login.html";
  } else if (currentPathname.includes("analizador-stl")) {
    window.location.href = pathPrefix + "login.html";
  }
}
